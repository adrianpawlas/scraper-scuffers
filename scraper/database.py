...
class SupabaseDB:
        ...
        try:
            ...
            for product in products:
                ...
                if formatted_product:
                    # Create unique key for deduplication based on source and external_id
                    dedup_key = f"{formatted_product.get('source')}:{formatted_product.get('external_id')}"
    ...
    def _format_product_for_db(self, product: Dict[str, Any]) -> Optional[Dict[str, Any]]:
        ...
        try:
            ...
            # Generate external_id from source and product URL
            import hashlib
            id_hash = hashlib.md5(f"{source}:{product_url}".encode()).hexdigest()
            external_id = f"{source}_{id_hash[:16]}"

            # Build the formatted product (id will be auto-generated by database)
            formatted = {
                'source': source,
